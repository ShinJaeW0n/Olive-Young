...
public class PrdDAO {
	...
	public Connection getConnect(){
		String dbURL="jdbc:mysql://localhost/young?serverTimezone=UTC&useSSL=false";
		String dbID="shjaewon";
		String dbPW="twd9696^^";
		try {
	...
	public ArrayList<Prd> selectPrdList() {
		...
		try {
			conn=getConnect();
			pstmt = conn.prepareStatement("SELECT * FROM young.product");
	...
	public Prd selectPrd(int id) {
		...
		try {
			...
			pstmt = conn.prepareStatement("SELECT * FROM young.product where id=?");
			pstmt.setInt(1, id);
			rs = pstmt.executeQuery();
	...
	public boolean insertOrder(Order order) {
		...
		try {
			...
			String sql = "insert into young.order values (?,?,?,?)";
	...
	public ArrayList<Order> getOrderList() {
		...
		try {
			conn = getConnect();
			String sql = "select * from young.order";
	...
	//전체 글의 개수를 가져오는 메소드.
			...
			public boolean deleteOrder(int num)
			{
				...
				try {
					conn = getConnect();
					conn.setAutoCommit(false);
					String sql = "delete from young.order where order_num=?";
